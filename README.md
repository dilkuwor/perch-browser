...
- **Streaming.** Picture quality, applied live (overrides `JPEG_QUALITY`). This is the ceiling: on a link that cannot keep up, the server temporarily sends fewer frames, a smaller picture and, last, a lower quality, and returns to your setting once it can.
- **About.** Home IP, server build, and a reset button.

### Latency

The speedometer icon in the title bar opens a panel with live connection stats: round-trip time to the home server, frames per second, bandwidth, the remote viewport size, and what is being streamed: the picture quality, and — marked *auto* when the server has stepped down for a slow link — whether only every second or third frame is sent and at what size. Its needle follows the round trip: right and green under ~80 ms, upright and amber under ~200 ms, left and red above — handy for telling an unresponsive site apart from a slow link.

### Phones and tablets

- One finger drags to scroll, a still tap clicks, and a long press right-clicks.
- Tapping a text field raises the on-screen keyboard automatically, and tapping elsewhere puts it away. Phones only open the keyboard *during* a tap, and on most links a round trip to the home server takes longer than a tap, so Perch does not wait for one: the server keeps the phone supplied with a map of where the page's text fields are (refreshed as the page loads, changes and scrolls), and the phone decides the moment your finger lifts. The server then checks what the page really focused and takes the keyboard down again if the tap turned out to hit something else. Fields hidden behind an open dialog are left out of the map, and a tap on the wide "bar" many sites draw around a narrow search field (Google's, for one) counts as a tap on the field — if the page focuses nothing in response, Perch focuses the field for you. While the keyboard is open the page shrinks to the space above it and the field you are typing into is kept in view. The keyboard button in the toolbar raises it manually — useful for fields inside cross-origin iframes, where the server cannot see what is there.
...
- **The picture adapts to the link.** When a device's connection cannot keep up (frames have to be held back), the server steps the stream down a ladder of cheaper encodings — first every second frame, then a smaller picture, and only then a lower JPEG quality, since blocky text hurts more than a softer picture — and climbs back once the link has been calm for a few seconds. The worst *watching* device sets the pace; one struggling phone does not go unnoticed, and a device in the background is not counted. Steps are spaced a few seconds apart, because each one restarts the capture. The Latency panel shows what is in use.
- **Nothing is streamed to a screen nobody is looking at.** When Perch is in a background tab, or the phone app is in the background, that device stops receiving pictures; when no device is watching at all, the home server stops capturing and encoding. Sound keeps playing, and the picture is current again the moment you return — any tap or key press counts as returning, so a phone that missed the "visible" event is never left blank.
- **Scrolling is sent once per display frame.** A trackpad fires 60–120 wheel events a second; they are summed and sent once per frame, so the same scroll uses a fraction of the messages and a click can never overtake it.
